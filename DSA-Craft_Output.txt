...
public class PuneMetro {
    ...
    public int vertexNum(String name) {
        return stations.size();
    }

    public boolean containStation(String name) {
    ...
    public int edgeNum(String name) {
    ...
    public boolean containEdge(String name1, String name2) {
    ...
    public boolean hasPath(String name1, String name2, HashMap<String, Boolean> visit) {
    ...
    public void displayMap() {
        ...
        for (String key : keys) {
            String str = key + "==>\n";
            Vertex v = stations.get(key);
            ArrayList<String> v_num = new ArrayList<>(v.neighbour.keySet());
            for (String num : v_num) {
                str = str + "\t" + num + "\t";
                if (num.length() < 16)
                    str = str + "\t";
                if (num.length() < 8)
                    str = str + "\t";
                str = str + v.neighbour.get(num) + "\n";
            }
    ...
    public static void main(String[] args) throws IOException {
        ...
        do {
            ...
            switch(choice) {
                ...
                case 3:
                {
                    // Create a list of station keys for user reference
                    ArrayList<String> keys = new ArrayList<>(stations.keySet());
                    printCodelist(); // Display the list of stations for user reference
                    System.out.println("Enter 1 :");
                    int ch = Integer.parseInt(input.readLine()); // Read user choice for input mode
                    String src = "", dst = ""; // Initialize variables for source and destination stations
                    System.out.println("Enter the SOURCE and DESTINATION Station's Serial Number");

                    // Process user choice
                    if (ch == 1) {
                        // Read the serial numbers of source and destination stations
                        src = keys.get(Integer.parseInt(input.readLine()) - 1);
                        dst = keys.get(Integer.parseInt(input.readLine()) - 1);
                    } else {
                        System.out.println("Invalid choice"); // Inform user of invalid choice
                        System.exit(0); // Exit the program as choice is invalid
                    }

                    HashMap<String, Boolean> h = new HashMap<>(); // Initialize a hashmap for path checking

                    // Check if source and destination stations are valid and a path exists
                    if (!p.containStation(src) || !p.containStation(dst) || !p.hasPath(src, dst, h)) {
                        System.out.println("THE INPUTS ARE INVALID"); // Inform user of invalid inputs
                    } else {
                        // Calculate and display the shortest distance using Dijkstra's algorithm
                        System.out.println("Shortest DISTANCE from " + src + " TO " + dst + " IS-----> "
                                + p.dijkstra(src, dst, false) + "m\n");
                    }
                ...
                case 4:
                {
                    ...
                    // Read the serial numbers of source and destination stations
                    String str1 = keys.get(Integer.parseInt(input.readLine()) - 1);
                    String str2 = keys.get(Integer.parseInt(input.readLine()) - 1);

                    HashMap<String, Boolean> processed1 = new HashMap<>(); // Initialize a hashmap for path checking

                    // Check if source and destination stations are valid and a path exists
                    if (!p.containStation(str1) || !p.containStation(str2) || !p.hasPath(str1, str2, processed1)) {
                        System.out.println("THE INPUTS ARE INVALID"); // Inform user of invalid inputs
                    } else {
                        // Calculate the shortest time using Dijkstra's algorithm and display it
                        // speed -> 95km/h -> 27m/s | distance calculated is in metres
                        int distanceInMeters = p.dijkstra(str1, str2, false);
                        double totalTimeSeconds = (double) distanceInMeters / 27; // Convert distance to seconds based on speed
                        double totalTimeMinutes = totalTimeSeconds / 60; // Convert seconds to minutes
                        System.out.println("Shortest Time from " + str1 + " TO " + str2 + " IS-----> "
                ...
                case 5:
                {
                    System.out.println("ENTER THE SOURCE AND DESTINATION STATIONS"); // Prompt user for source and destination stations
                    String s1 = input.readLine(); // Read the source station
                    String s2 = input.readLine(); // Read the destination station
                    HashMap<String, Boolean> processed2 = new HashMap<>(); // Initialize a hashmap for path checking

                    // Check if source and destination stations are valid and a path exists
                    if (!p.containStation(s1) || !p.containStation(s2) || !p.hasPath(s1, s2, processed2))
                        System.out.println("THE INPUTS ARE INVALID"); // Inform user of invalid inputs
                    else
                    {
                        // Get the minimum distance path between source and destination stations
                        ArrayList<String> str = p.get_Interchanges(p.getMinimumDistance(s1, s2));
                        int len = str.size();
                        System.out.println("------------------------------------");
                        System.out.println("SOURCE STATION : " + s1);
                        System.out.println("DESTINATION STATION : " + s2);
                        System.out.println("DISTANCE : " + str.get(len - 1)); // Display the distance of the shortest path
                        System.out.println("~~~~~~~~~~~~~");
                        System.out.println("START ==> " + str.get(0)); // Display start station
                        for (int i = 1; i < len - 3; i++)
                            System.out.println(str.get(i)); // Display intermediate stations
                        System.out.print(str.get(len - 3) + " ==> END"); // Display end station
                        System.out.println("\n~~~~~~~~~~~~~");
                    }
                    break; // Exit the switch case
                }
                ...
                //      double latitude = Double.parseDouble(input.readLine());
                //      System.out.println("Enter longitude:");
                //      double longitude = Double.parseDouble(input.readLine());
                //      getRealTimeTrafficUpdate(latitude, longitude);
                break;
